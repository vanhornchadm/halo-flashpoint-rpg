...
**Type I** groups passive and support items; **Type II** groups activated devices; **Type III** groups ordnance. These categories are not power tiers or automatic inventory-slot limits. Each profile determines its actual operation.

Agree with the GM on a suitable loadout, carried quantities, and compatible ammunition, charges, medical supplies, and repair parts. These rules do not grant every listed device at character creation or establish a new point-buy budget. Species, physical access, carrying capacity, and equipment compatibility still apply; an AI does not acquire hands by owning a device.

## Activation and Timing

- Activating equipment normally costs a **short Auxiliary action during your activation**.
- Passive effects require no action and trigger exactly as stated.
- Equipment that explicitly modifies a move or attack is declared at its stated timing without an additional action cost. It does not provide the underlying move or attack.
- Grenades require a **Shoot action**, following [[keywords/grenade|Grenade]].
- Free actions, long actions, and effects outside your activation require explicit permission in the item entry. A free action does not grant another activation.

Equipment costs no Command Die unless an effect explicitly says otherwise. It is separate from [[rules/special-orders|Special Orders]] and does not remove their limits. Normal action, terrain, engagement, and Break Away rules apply unless the item creates a specific exception. Resolve an attack or test completely before another voluntary equipment use unless an explicit trigger interrupts it.

Check legal targets and declare choices before spending the use. Once activated, a failed test or miss consumes the use. Different applicable effects combine; identical keywords or copies of the same equipment effect do not stack unless explicitly permitted by [[rules/keyword-conventions|Reading Keywords]] or the item.

## Uses, Charges, and Duration
...
Equipment can make an approach possible, remove the need for a roll, or change the difficulty. Follow [[rules/roleplay|Light Roleplay Rules]]: roll only when the outcome is uncertain and failure matters. A grappleshot can reach a suitable ledge; using it under fire while carrying a casualty may require a test and suitable carrying capability.

Before using a device whose duration is expressed in combat rounds or activations outside combat, establish the brief task or scene it covers. This changes how duration is described, not how many charges are spent or what the equipment can accomplish. Starting a new scene does not refresh a spent device.
